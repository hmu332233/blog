...
팀 리딩에 대한 부분은 이전에 작성한 글에 작성해 두었고  
...
- [Q.Link](http://Q.Link) - Hide your link behind a simple quiz.
...
처음에는 vercel에 배포하여 api까지 모두 관리하려고 했으나, 그 때 당시 상황으론 첫 진입이 느려서 aws에 api를 별도로 만들어두는 것으로 진행하게 되었다.
...
내 mongodb는 한국 리전에 있었기 때문에 api 요청이 대충 이런 흐름을 타게되었고, cold start이기 때문에 첫 페이지 진입 시간이 너무 길어져서, api 서버를 aws로 별도로 띄워서 적어도 내 서비스 내의 레이턴시를 줄이려고했다.
